...
class CarlaDataset(object):
    ...
    def prepro(self):
        ...
        input_folder= sorted(glob(os.path.join(self.root, "preprocessed",'bev_rgb_acc_inpaint-*'))):
            for input_image_dir in input_folder:
                label_folder_index= input_image_dir[20:26]
                label_file_index= inputimage_dir_[27:33]
    ...
    def __getitem__(self, idx):
        ...
        g_centers, r_centers = self.get_binimg(rec)

        img= Image.open(self.ixes[idx]['input_dir']).convert("RGB")
        target= self.get_keypoints()
